...
import gobblin.metastore.util.JdbcUrl;


class TestMetastoreDatabase implements Closeable {
  ...
  TestMetastoreDatabase() throws Exception {
  ...
  JdbcUrl getJdbcUrl(String database) throws URISyntaxException {
  ...
  private JdbcUrl getBaseJdbcUrl() throws URISyntaxException {
    return JdbcUrl.create()
        .setScheme("mysql")
        .setHost("localhost")
        .setPort(config.getPort());
  }

  private JdbcUrl getInformationSchemaJdbcUrl() throws URISyntaxException {
    return getBaseJdbcUrl()
        .setPath(INFORMATION_SCHEMA)
        .setUser(ROOT_USER);
  }

  private Optional<Connection> getConnector(JdbcUrl jdbcUrl) throws SQLException {
  ...
  void prepareDatabase(String database)
      throws Exception {
