...
public class ParallelRunner implements Closeable {
  ...
  private void attemptWithRetries(Callable<Void> callable, int maxAttempts, int backoffMilliseconds) throws Exception {
    int attempt = 0;
    while (true) {
      Exception exception;
      try {
      ...
      if (++attempt < maxAttempts) {
        Thread.sleep(attempt * backoffMilliseconds);
      } else {
        throw exception;
      }
    }
