...
public abstract class Id {
    ...
    static final Pattern PATTERN = Pattern.compile("\\A([^" + SEPARATOR + "]+)" + SEPARATOR + "(.+)" + SEPARATOR + "(\\d+)\\z");
    ...
    public static Id parse(String id) {
        ...
        throw new RuntimeException(String.format("Invalid id: %s", id));
    }
    ...
    public static class Job extends Id {
        ...
        public static Job parse(String id) {
            Matcher matcher = Job.PATTERN.matcher(id);
            if (matcher.matches() && Job.PREFIX.equals(matcher.group(1))) {
                return new Job(matcher.group(2), matcher.group(3));
            }
            throw new RuntimeException(String.format("Invalid %s id: %s", PREFIX, id));
        }
    }
    ...
    public static class Task extends Id {
        ...
        public static Task parse(String id) {
            Matcher matcher = Task.PATTERN.matcher(id);
            if (matcher.matches() && Task.PREFIX.equals(matcher.group(1))) {
                return new Task(matcher.group(2), matcher.group(3));
            }
            throw new RuntimeException(String.format("Invalid %s id: %s", PREFIX, id));
        }
    ...
    public static class MultiTask extends Id {
        ...
        public static MultiTask parse(String id) {
            Matcher matcher = MultiTask.PATTERN.matcher(id);
            if (matcher.matches() && MultiTask.PREFIX.equals(matcher.group(1))) {
                return new MultiTask(matcher.group(2), matcher.group(3));
            }
            throw new RuntimeException(String.format("Invalid %s id: %s", PREFIX, id));
        }
